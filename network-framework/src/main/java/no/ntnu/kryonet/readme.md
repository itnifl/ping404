...
```gradle
dependencies {
    implementation project(':network-framework')
}
```
...
client.sendUDP(new no.ntnu.kryonet.packets.Ping(1));
```

## Notes

- Prefer builders as the public entry point
- Internal transport classes are in `no.ntnu.kryonet.internal`
